...
- [x] **11.6a Part A** Visual token reconciliation: old colors/fonts none found, vermilion guard fixed (onboarding error text), hardcoded colors migrated to CSS vars, error/bergabung/profil pages reconciled to lp-* tokens (2026-06-13). buat-partai wizard flagged for Phase 1 — 22 gray/slate instances in complex multi-step form; functional clarity over visual purity.
...
- [x] **12.9** Follow-ups resolved (2026-06-13): CQ1 merged to single CTA, CQ2 kept abstract, CQ3 trimmed, CQ4 flagged as maintenance coupling; TQ1–TQ4 all decided (TQ1 /jelajah, TQ2 SSR+no-store, TQ3 deferred, TQ4 per-subdomain); apex `apps/www` migrated to shared `@alternatif/ui/landing.css` (~560 lines removed)
